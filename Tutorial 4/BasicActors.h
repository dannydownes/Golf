...
namespace PhysicsEngine
{
	...
	class DBox : public DynamicActor
	{
	public:
		...
		// - denisty: 1kg/m^3
		DBox(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
	...
	class Cross : public DynamicActor
	{
	public:
		Cross(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
			CreateShape(PxBoxGeometry(dimensions), density);
	...
	public:
		Windmill(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(.2f, .3f, 0.8f), density);	// bottom right
			CreateShape(PxBoxGeometry(.2f, .3f, 0.8f), density);	// bottom middle
			CreateShape(PxBoxGeometry(.2f, .3f, 0.8f), density);	// bottom left
			CreateShape(PxBoxGeometry(1.f, 1.f, 0.8f), density);		// tower
		}
	};

	class Straight : public StaticActor
	{
	public:
		Straight(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(0.2f, 0.4f, 1.0f), density); // left
			CreateShape(PxBoxGeometry(1.f, 0.3f, 1.0f), density); // bottom
			CreateShape(PxBoxGeometry(0.2f, 0.4f, 1.0f), density); // right
		}
	};
